...
@Uninterruptible public final class TraceGenerator {
  ...
  /* Fields for tracing */
  private static SortTODSharedDeque tracePool; // Buffers to hold raw nurseryTrace
  private static TraceBuffer trace;
  private static boolean traceBusy; // If we are building the nurseryTrace
   ...
   * objects to the nurseryTrace processor.
   *
   * @param worklist_ The dequeue that serves as the worklist for
   * death time propagation
   * @param trace_ The dequeue used to store and then output the nurseryTrace
   */
  @Interruptible
   ...
   * any remaining information in the nurseryTrace buffer.
   *
  ...
  /**
   * Add the information in the bootImage to the nurseryTrace.  This should be
  ...
  public static void boot(Address bootStart) {
    ...
    while (!trav.isNull()) {
      ...
      /* Add the boot image object to the nurseryTrace. */
   ...
   * will add the pointer store to the nurseryTrace buffer and, when Merlin lifetime
  ...
  public static void processPointerUpdate(boolean isScalar,
    ...
    // The nurseryTrace can be busy only if this is a pointer update as a result of
    ...
    if (!traceBusy) {
      ...
      traceBusy = true;
      /* Add the pointer store to the nurseryTrace */
   ...
   * object allocation to the nurseryTrace buffer, triggers the necessary collection
   * work at exact allocations, and output the data in the nurseryTrace buffer.
   *
  ...
  public static void traceAlloc(boolean isImmortal, ObjectReference ref,
    ...
    /* Add the allocation into the nurseryTrace. */
    traceBusy = true;
    /* When legally permissible, add the record to the nurseryTrace buffer */
    if (MERLIN_ANALYSIS) {
  ...
  /**
   * This computes and adds to the nurseryTrace buffer the unreachable time for
      ...
      while (!thisRef.isNull()) {
        ...
        if (getTraceLocal().isReachable(thisRef)) {
          ...
          if (MERLIN_ANALYSIS)
            ...
            deadTime = lastGC;
          /* Add the death record to the nurseryTrace for unreachable objects. */
   ...
   * to the current nurseryTrace time.
   *

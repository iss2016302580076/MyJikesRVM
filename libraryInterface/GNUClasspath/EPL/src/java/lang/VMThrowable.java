...
public final class VMThrowable {
  /** The stack nurseryTrace for this throwable */
  private final StackTrace stackTrace;

  /**
   * Zero length array of stack nurseryTrace elements, returned when handling an OOM or
   * an unexpected error
   */
  ...
  static VMThrowable fillInStackTrace(Throwable parent) {
    ...
    try {
      ...
      VM.sysFail("VMThrowable.fillInStackTrace(): Cannot fill in a stack nurseryTrace; got a weird Throwable when I tried to");
      return null;
    }
  }

  /** Return the stack nurseryTrace */
  StackTraceElement[] getStackTrace(Throwable parent) {
    ...
    for (StackTrace.Element vmElement : vmElements) {
      if (vmElement == null) {
        VM.sysWriteln("Error stack nurseryTrace with null entry");
        RVMThread.dumpStack();
        return zeroLengthStackTrace;
      }

...
public final class RVMThread extends ThreadContext {
  ...
  /**
   * An integer token identifying the last stack nurseryTrace request
   */
  int shouldBlockForStackTraceToken;

  /**
   * Should the thread block so that another thread can get a stack nurseryTrace for it?
   */
  boolean shouldBlockForStackTrace;

  /**
   * Is the thread blocked because another thread wants to get a stack nurseryTrace for it?
   */
  ...
  private void handleDebugRequestForThread() {
    ...
    if (!isAboutToTerminate) {
      ...
      if (isInJava()) {
        ...
        takeYieldpoint = 1;
        VM.sysWriteln("(stack nurseryTrace will follow if thread is not lost...)");
      } else {
        if (contextRegisters != null) {
          dumpStack(contextRegisters.getInnermostFramePointer());
        } else {
          VM.sysWriteln("(cannot dump stack nurseryTrace; thread is not running in Java but has no contextRegisters)");
        }
      }
    }
    dumpLock.unlock();
  ...
  void handleHandshakeRequest() {
    ...
    if (asyncDebugRequestedForThisThread) {
      ...
      dumpLock.lockNoHandshake();
      VM.sysWriteln("Handling async stack nurseryTrace request...");
      dump();
      VM.sysWriteln();
      dumpStack();
      dumpLock.unlock();
...
*  */
  ...
  */
  ...
  // same as nurseryTrace, but prints integer value in hex
  //
   ...
   * @param lineNumber the line number for the stack nurseryTrace
   * @param fp the frame pointer of the method's frame
